...
- **Notifications**: Discord channel webhook via Supabase Database Webhook → `/api/discord-webhook`; personal inbox via `user_notifications` table + bell in TopBar
...
app/
  ...
  (app)/                    Auth-gated routes with TopBar+BottomNav shell
    ...
    notifications/            Personal notification inbox (client page, day-bucketed,
                              All/Signals filter, mark-one + mark-all-read)
  signal/[id]/              Public Signal detail (outside (app) — shareable link)
  api/og/signal/[id]/       Signal Card image (story 1080x1920, feed 1080x1350,
                              preview 1200x630) via next/og ImageResponse
  api/signals/evaluate/     Supabase DB Webhook receiver → engine (loop-guarded)
  api/signals/backfill/     Admin-only silent historical sweep
components/
  signals/                  SignalList (profile chips), CopySignalLink
  notifications/            NotificationBell (TopBar popover), NotificationRow
...
lib/
  ...
  signals.ts                Public read API (listUserSignals, getUserSignal, listSignalDefinitions)
  notifications.ts          Server-side reads (getUnreadCount, listNotifications)
  signals/metrics.ts        11 countable metrics (books, events, manifests, etc.)
  signals/engine.ts         evaluateUser() — idempotent unlock + notifications + announce
...
| 0027 | collective_signals     | ⏳ pending (apply before going live) | `signal_definitions` + `signal_rules` + `user_signals` tables. Extends `activity_log` CHECK with `SIGNAL_UNLOCKED` type + `user_signal_id` FK. Seeds 12 v0 Signal definitions + rules. RLS: public-read on definitions/user_signals; service-role-only writes.                                                          |
| 0028 | user_notifications     | ⏳ pending (apply before going live) | `user_notifications` private inbox. RLS: select_own + update_own (recipient = auth.uid()). UNIQUE(recipient_user_id, type, object_type, object_id) for dedupe. Partial index on (recipient_user_id, read_at) for fast unread count.                                                                                     |
...
| 6   | Apply migrations 0027 + 0028 to production Supabase                                                                                   | Supabase Dashboard → SQL editor                 | Yes — engine + notifications won't work until applied                    |
| 7   | Add `SIGNALS_WEBHOOK_SECRET=<random>` to Vercel env vars                                                                              | Vercel → Settings → Env                         | Yes — evaluate route auth gate needs this                                |
| 8   | Create 2nd Supabase DB webhook: `activity_log` INSERT → `/api/signals/evaluate` with `Authorization: Bearer <SIGNALS_WEBHOOK_SECRET>` | Supabase Dashboard → Database → Webhooks        | Yes — engine won't receive events until live                             |
| 9   | Run `POST /api/signals/backfill` once after migrations applied                                                                        | curl or browser (admin session required)        | No — historical users won't have Signals until sweep runs                |
...
- **~~Collective Signals + Personal Notification Center~~** ✅ shipped (migrations pending prod) — Auto-unlock achievement system (Strava/Apple Fitness vibe). Event-sourced: `activity_log` → DB Webhook #2 → `/api/signals/evaluate` → `signal_definitions` + `signal_rules` → `user_signals` (idempotent upsert). Engine: `lib/signals/engine.ts` `evaluateUser()` — computes 11 metrics via `lib/signals/metrics.ts`, writes `user_signals` + `user_notifications` (private, every unlock) + `activity_log SIGNAL_UNLOCKED` (public, announce=true signals only). Discord "Signal Drop" reuses existing webhook route (SIGNAL_UNLOCKED type, gold embed + card preview image). Signal Cards via `next/og` at `/api/og/signal/[id]?format=story|feed|preview`. Public detail at `/signal/[id]` (shareable, outside `(app)` auth gate). Profile shows Signals as warm sepia chips between stats and shelf. Notification Center: bell in TopBar, unread count, popover + `/notifications` full page (day-bucketed, All/Signals filter). Schema: `0027_collective_signals.sql` + `0028_user_notifications.sql` — **not applied to prod yet** (see outstanding items #6-#9). Loop guard: evaluate route skips `SIGNAL_UNLOCKED` rows. Backfill: `POST /api/signals/backfill` (admin, silent, idempotent).
