...
export function MemberCard({ member }: { member: MemberSummary }) {
  ...
  // Discussion first — connection-moment signal leads transactional ones.
  if (member.open_for_discussion) openTags.push("Diskusi");
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {member.profession && (
            <p className="mt-1 text-caption text-ink-soft line-clamp-1">{member.profession}</p>
          )}
        </div>
      </div>

      {member.bio && <p className="text-body-sm text-ink-soft line-clamp-2">{member.bio}</p>}
      ...
      <div className="mt-auto pt-2 flex items-center justify-between gap-2 border-t border-hairline-soft">
        <span className="text-caption text-muted">
          <span className="font-semibold text-ink">{member.book_count}</span> buku
        </span>
        {openTags.length > 0 && (
