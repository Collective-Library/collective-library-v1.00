...
export default async function AdminFeedbackPage({
  ...
  let query = supabase
    .from("feedback")
    .select(
      "*, user:profiles_public!feedback_user_id_fkey(full_name, username)",
    )
  ...
  type Row = FeedbackRowWithUser & {
    user: { full_name: string | null; username: string | null } | { full_name: string | null; username: string | null }[] | null;
  };
      ...
      {rows.length === 0 ? (
        <div className="rounded-card-lg border border-hairline bg-paper p-10 text-center">
          ...
          <p className="mt-2 text-body text-muted">
            Gak ada feedback yang cocok di filter ini.
          </p>
        </div>
      ) : (
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        active ? "bg-ink text-parchment" : "bg-paper text-ink-soft border border-hairline hover:bg-cream",
      )}
    >
      {label}
    </Link>
